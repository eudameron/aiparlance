...
Phase C of the roadmap delivered the **first official emitters** (transpilers) for AI Parlance. They live in the monorepo as Preview targets — not npm-published packages yet, but real AST → artifact pipelines you can run with `aip emit`.
...
PostgreSQL was the **first** emitter because the docs treat it as the primary SQL dialect. OpenAPI, TypeScript, and Go completed the first multi-target wave; MySQL and the remaining matrix targets followed as Preview MVPs.
...
One validated spec feeds all Preview emitters. Naming (snake_case, FK `*_id`, plural tables) stays aligned across SQL dialects, OpenAPI, and Go `json` tags.
...
Full install and a complete CRUD story: [Getting started](/en/getting-started) · [CRUD walkthrough](/en/crud-walkthrough).
...
- Full-tier examples **parse and validate**; not every Behavior/Security detail is reflected in every emitter yet.
...
Matrix status: [Specification § Transpiler matrix](/en/specification#transpiler-matrix).
