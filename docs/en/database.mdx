...
|---|---|---|
| PostgreSQL | `postgres` | Preview (`aip emit sql` — Core DDL) |
| MySQL | `mysql` | Planned |
| SQLite, MariaDB, MongoDB, … | Roadmap | — |

You can declare `database mysql` in the spec today; MySQL DDL generation is not shipped yet. PostgreSQL Core DDL: `aip emit sql` ([transpiler matrix](/en/specification#transpiler-matrix)).
