...
**Toolchain (v0.1):** Core **parser**, **validator**, **PostgreSQL DDL**, **OpenAPI 3**, **TypeScript**, and **Go** emitters are in the monorepo (`aip parse` / `aip validate` / `aip emit sql|openapi|typescript|go`). Other matrix emitters are not published yet. This document is the normative prose source; see [spec/v0.1/grammar.ebnf](https://github.com/eudameron/aiparlance/blob/main/spec/v0.1/grammar.ebnf) and [ROADMAP.md](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md).
...
|---|---|---|
| PostgreSQL | DDL, indexes (Core entities) | Preview (`aip emit sql`) |
| MySQL | DDL, migrations | Planned (same `database mysql` in `app`; no emitter yet) |
| Go | structs, handlers, JWT middleware | Preview (`aip emit go`) |
| TypeScript | interfaces, guards | Preview (`aip emit typescript`) |
| Python | models, decorators | Planned |
| PHP | classes, policies | Planned |
| OpenAPI | paths, schemas, security | Preview (`aip emit openapi`) |
