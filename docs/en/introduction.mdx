...
<Note>
**v0.1 is still a draft language.** A Core **parser**, **semantic validator**, and emitters for **PostgreSQL DDL**, **OpenAPI 3**, **TypeScript**, and **Go** ship in the monorepo (`aip parse` / `aip validate` / `aip emit sql|openapi|typescript|go` — see [ROADMAP](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md)). Other matrix emitters are not published yet. This documentation remains the normative source for the language.
</Note>
