...
<Note>
**v0.1 is still a draft language.** A Core **parser**, **semantic validator**, **PostgreSQL DDL emitter**, and **OpenAPI 3 emitter** ship in the monorepo (`aip parse` / `aip validate` / `aip emit sql|openapi` — see [ROADMAP](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md)). Official **TypeScript / other emitters** are not published yet. This documentation remains the normative source for the language.
</Note>
