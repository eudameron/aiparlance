...
|---|---|---|
| PostgreSQL | `postgres` | Prévia (`aip emit sql` — DDL, índices, seeds) |
| MySQL | `mysql` | Prévia (`aip emit mysql`) |
| SQLite, MariaDB, MongoDB, … | Roadmap | — |

PostgreSQL: `aip emit sql`. MySQL: declare `database mysql` e rode `aip emit mysql` (ver [`examples/mysql-minimal.aip`](https://github.com/eudameron/aiparlance/blob/main/examples/mysql-minimal.aip)). Matriz: [Especificação § Matriz de transpiladores](/pt/specification#transpiler-matrix).
