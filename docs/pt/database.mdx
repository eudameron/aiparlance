...
|---|---|---|
| PostgreSQL | `postgres` | Prévia (`aip emit sql` — DDL Core) |
| MySQL | `mysql` | Planejado |
| SQLite, MariaDB, MongoDB, … | Roadmap | — |

É válido declarar `database mysql` na spec; geração de DDL MySQL ainda não foi publicada. DDL Core PostgreSQL: `aip emit sql` ([matriz de transpiladores](/pt/specification#transpiler-matrix)).
