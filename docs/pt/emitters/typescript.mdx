...
description: "@aiparlance/typescript — papel app, nota 27/55, objetivos pass/partial/fail"
---
...
Interfaces, type guards, schemas Zod (peer `zod`), helpers de path com `api.prefix`, consts de policy e `createCrudApp()` / `listenCrudApp()` em memória (validação, soft-delete, headers de auth).
...
Golden `minimal.ts` + saída emitida typecheck isolada. CI de examples.
