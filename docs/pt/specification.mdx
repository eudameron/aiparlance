...
**Toolchain (v0.1):** **parser** e **validador** Core estão no monorepo (`packages/` — `aip parse` / `aip validate`). **Transpiladores/emitters** ainda não publicados (matriz abaixo permanece Planejado). Este documento é a fonte normativa em prosa; gramática em [spec/v0.1/grammar.ebnf](https://github.com/eudameron/aiparlance/blob/main/spec/v0.1/grammar.ebnf) e marcos em [ROADMAP.md](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md).
