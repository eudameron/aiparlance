...
|---|---|---|
| PostgreSQL | DDL, índices (entidades Core) | Prévia (`aip emit sql`) |
| MySQL | DDL, migrations | Prévia (`aip emit mysql`) |
| Go | structs, handlers, middleware JWT | Prévia (`aip emit go`) |
| TypeScript | interfaces, guards | Prévia (`aip emit typescript`) |
| Python | models, decorators | Prévia (`aip emit python`) |
| PHP | classes, policies | Prévia (`aip emit php`) |
| OpenAPI | paths, schemas, security | Prévia (`aip emit openapi`) |
