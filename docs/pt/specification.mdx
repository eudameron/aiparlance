...
**Toolchain (v0.1):** **parser**, **validador**, e emitters **PostgreSQL (DDL)**, **OpenAPI 3**, **TypeScript** e **Go** Core estão no monorepo (`aip parse` / `aip validate` / `aip emit sql|openapi|typescript|go`). Demais emitters da matriz ainda não publicados. Este documento é a fonte normativa em prosa; gramática em [spec/v0.1/grammar.ebnf](https://github.com/eudameron/aiparlance/blob/main/spec/v0.1/grammar.ebnf) e marcos em [ROADMAP.md](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md).
...
| MySQL | DDL, migrations | Planejado (`database mysql` no `app`; sem emitter ainda) |
