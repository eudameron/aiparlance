...
**Ferramentas de referência (v0.1):** **parser**, **validador** v0.1 completo (Core + Infra + Security + Behavior) e emitters em prévia na matriz (`aip parse` / `aip validate` / `aip emit …`) estão no monorepo. Este documento é a fonte normativa em prosa; gramática em [spec/v0.1/grammar.ebnf](https://github.com/eudameron/aiparlance/blob/main/spec/v0.1/grammar.ebnf) e marcos em [ROADMAP.md](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md).
...
Gramática normativa completa: [spec/v0.1/grammar.ebnf](https://github.com/eudameron/aiparlance/blob/main/spec/v0.1/grammar.ebnf). Resumo:
...
Exatamente um `app_block` é obrigatório e deve aparecer primeiro. Ordem de modificadores: `required` | `optional` → `unique` → `default(...)`.
...
|---|---|---|
| **Core** | `app`, `entity`, `crud`, validação inline e bloco `validation`; tipos `enum(…)` inline; relações `belongs_to` inline | Estável |
...
| **Behavior** | `workflow`, `event`, `lifecycle`, `job`, `queue`, `ai_context`; statements em workflow (`emit`, `create`, …) | Beta |

Blocos de topo `enum { }` / `relation { }` e `has_one` / `has_many` / `many_to_many` estão no **roadmap**, não no v0.1.

Blocos beta podem mudar entre releases v0.x menores. O Core é estável dentro do v0.1; a linguagem permanece **rascunho** (ferramentas de referência em prévia no monorepo — ainda não é v1.0 congelada). Ver [ROADMAP](https://github.com/eudameron/aiparlance/blob/main/ROADMAP.md).
...
|---|---|---|
| PostgreSQL | DDL, índices, seeds (Core + Infra) | Prévia (`aip emit sql`) · Phase D: migrations |
| MySQL | DDL, índices, seeds | Prévia (`aip emit mysql`) |
| Go | structs, handlers, stubs de middleware JWT | Prévia (`aip emit go`) |
| TypeScript | interfaces, guards | Prévia (`aip emit typescript`) · Phase D: aprofundar |
| Python | models / dataclasses MVP | Prévia (`aip emit python`) |
| PHP | classes MVP | Prévia (`aip emit php`) |
| OpenAPI | paths, schemas, security schemes | Prévia (`aip emit openapi`) · Phase D: policy |
| Docs | Markdown / referência API | Prévia (`aip emit docs`) |
| Testes | fixtures CRUD | Prévia (`aip emit tests`) |
| Workers | filas, jobs | Prévia (`aip emit workers`) |
...
| Arquivo | Papel |
|---|---|
| [examples/minimal.aip](https://github.com/eudameron/aiparlance/blob/main/examples/minimal.aip) | Spec Core mínima válida |
...
| [examples/ops-reference.aip](https://github.com/eudameron/aiparlance/blob/main/examples/ops-reference.aip) | Extras Infra + Behavior (`seed`, `lifecycle`, `job`, …) |

Capítulos de domínio ([Banco](/pt/database), [Segurança](/pt/security), [Workflows](/pt/workflows)) documentam extensões em relação a este arquivo.
