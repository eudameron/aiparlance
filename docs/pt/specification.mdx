...
|---|---|---|
| PostgreSQL | DDL, índices, seeds, migrations init up/down | Prévia (`aip emit sql`) |
| MySQL | DDL, índices, seeds | Prévia (`aip emit mysql`) |
| Go | structs, handlers, stubs de middleware JWT | Prévia (`aip emit go`) |
| TypeScript | interfaces, Zod, CRUD em memória | Prévia (`aip emit typescript`) |
| Python | models / dataclasses MVP | Prévia (`aip emit python`) |
| PHP | classes MVP | Prévia (`aip emit php`) |
| OpenAPI | paths, schemas, security de policy | Prévia (`aip emit openapi`) |
