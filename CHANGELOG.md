...
- **Phase D2.5 P0 deepen:** TypeScript honors `api.cors` / `api.rate_limit`, unique→409, paginated lists, typed errors, `owner_or_manager` (admin|editor), `/health`, env for `DATABASE_URL`/`AIP_JWT_SECRET`
- **Phase D2.5 P0 deepen:** OpenAPI `x-aip-cors` / `x-aip-rate-limit`, soft-delete notes, pagination params, shared `Error` schema, 409/429
- **Phase D2.5 P0 deepen:** SQL `email` → `CITEXT`, soft-delete `{table}_active` views
...
- Re-scored after D2.5 (2026-08-08): PostgreSQL **19/23** (83%), OpenAPI **28/33** (85%), TypeScript **36/55** (65%) — scorecards, docs EN+PT, site badges
- Re-scored after D2 deepen (2026-08-08): PostgreSQL **17/23** (74%), OpenAPI **20/33** (61%), TypeScript **25/55** (45%) — superseded by D2.5
...
- Home toolchain cards: badge shows maturity score % (e.g. PostgreSQL 83%, OpenAPI 85%, TypeScript 65%, MySQL 61%)
